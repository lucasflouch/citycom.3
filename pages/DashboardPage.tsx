...
const DashboardPage: React.FC<DashboardPageProps> = ({ session, profile, onNavigate, data, refreshData }) => {
  ...
  const userPlan: SubscriptionPlan | undefined = useMemo(() => {
    ...
    // Fallback por si el plan_id es nulo o inválido, asignamos el gratis por defecto.
    return data.plans.find(p => p.precio === 0);
  }, [profile, data.plans]);

  // LÓGICA CORREGIDA:
  // Ahora, si el plan aún no se cargó (`userPlan` es undefined), asumimos que SÍ se puede publicar.
  // Esto previene el mensaje de error al cargar la página y estabiliza el estado.
  const canPublishMore = useMemo(() => {
    if (!userPlan) {
      return true; // Asumimos que sí se puede hasta que el plan se cargue y confirme lo contrario.
    }
