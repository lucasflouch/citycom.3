...
const MessagesPage: React.FC<MessagesPageProps> = ({ session, profile, appData, initialConversation }) => {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`messages`)
      .on('postgres_changes', { 
          event: '*', // Escuchar INSERTs y UPDATEs
          schema: 'public', 
          table: 'messages' 
      },
        (payload) => {
          ...
          if (payload.eventType === 'INSERT') {
            ...
            // Ignorar mensajes que yo mismo envié (ya se manejan con actualización optimista)
            if (newMsg.sender_id === session.user.id) return;
            
            // Si el mensaje es de la conversación activa, lo añadimos y marcamos como leído
            if (newMsg.conversation_id === activeConv?.id) {
              setMessages((prev) => [...prev, newMsg]);
              markConversationAsRead(newMsg.conversation_id, session.user.id);
            } else {
              // Si es para CUALQUIER otra conversación (existente o nueva),
              // recargamos la lista de conversaciones para reflejar el cambio.
              // Esto es simple, robusto y asegura que nuevas conversaciones aparezcan.
          ...
          if (payload.eventType === 'UPDATE') {
            ...
            // Si un mensaje actualizado pertenece a mi chat, actualizo su estado (para el 'leído')
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    if (sentMessage) {
        // ACTUALIZACIÓN OPTIMISTA: Añadir el mensaje a la UI inmediatamente.
